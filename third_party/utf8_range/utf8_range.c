...
static inline const char* utf8_range_SkipAscii(const char* data,
  ...
  /* Sixteen bytes per iteration. movemask gathers the high bit of every byte
     ...
     first non-ASCII byte directly. Fewer than sixteen bytes may remain, and the
     word loop below then covers eight of them at once -- strings that short are
     the common case, not a rare tail. */
  while (16 <= end - data) {
